# Apptainer image for plasmidCC (Fir Alliance cluster)
...
```bash
cd envs
tar -czf plasmidcc.tar.gz plasmidcc
scp plasmidcc.tar.gz fir:/home/$yourname/scratch
```
...
cd scratch
tar -xzf plasmidcc.tar.gz -C .
cd plasmidcc
```

Build the apptainer image on `fir` cluster:

```bash
module load apptainer
APPTAINER_BIND=" "
apptainer build apptainer_plasmidcc.sif apptainer_img.def
```

Test the apptainer image:

```bash
apptainer run -C apptainer_plasmidcc.sif plasmidCC --help
```

Move it to the environment directory:

```bash
mv apptainer_plasmidcc.sif /project/def-chauvec/wg-anoph/benchmarking/ENVS
```
