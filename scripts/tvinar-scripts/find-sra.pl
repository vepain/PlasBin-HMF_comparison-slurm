...
while (my $sampleid = <$samplesfile>) {
	...
	while (my $line = <$in>) {
	    chomp $line;
	    @parts = split ",",$line;
	    my ($run,$date,$platform,$species,$size) =
		($parts[$hi{'Run'}],$parts[$hi{'ReleaseDate'}],$parts[$hi{'Platform'}],$parts[$hi{'ScientificName'}],$parts[$hi{'size_MB'}] || 0);

	    print STDERR join("\t",$run,$platform,$size),"\n";
	    
	    $sample{$sampleid}{'species'} = $species;
	    if ($platform eq "ILLUMINA" ) {
		if ($size > $sample{$sampleid}{'sizeshort'}) {
		  $sample{$sampleid}{'short'} = $run;
		  $sample{$sampleid}{'numshort'} += 1;
		  $sample{$sampleid}{'sizeshort'} = $size;
		}
	    } elsif (($platform eq "OXFORD_NANOPORE") || ($platform eq "PACBIO_SMRT")) {
		if ($size > $sample{$sampleid}{'sizelong'}) {
		  $sample{$sampleid}{'long'} = $run;
		  $sample{$sampleid}{'numlong'} += 1;
		  $sample{$sampleid}{'longtype'} = $platform;
		  $sample{$sampleid}{'sizelong'} = $size;
		}
	    } else {
