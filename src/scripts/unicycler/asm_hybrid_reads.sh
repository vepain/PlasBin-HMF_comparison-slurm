...
# which SLURM wipes at the end of the task
reads_dir="$SLURM_TMPDIR/reads"
fastq_1_gz="$reads_dir/${sra_sr_id}_1.fastq.gz"
fastq_2_gz="$reads_dir/${sra_sr_id}_2.fastq.gz"
fastq_lr_gz="$reads_dir/${sra_lr_id}.fastq.gz"
#
# Outputs
#
...
fastq-dump --split-3 --outdir "$reads_dir" "$reads_dir/$sra_sr_id"

prefetch "$sra_lr_id" --output-directory "$reads_dir"
fastq-dump --split-3 --outdir "$reads_dir" "$reads_dir/$sra_lr_id"

gzip -f "${fastq_1_gz%.gz}" "${fastq_2_gz%.gz}" "${fastq_lr_gz%.gz}"
