...
#SBATCH --array=2-561
#SBATCH --output=logs/asm_hybrid_reads/%A/%a.out
#SBATCH --error=logs/asm_hybrid_reads/%A/%a.err

# ---------------------------------------------------------------------------- #
# User variables
# ---------------------------------------------------------------------------- #
declare -r samples_tsv="/project/6001426/wg-anoph/benchmarking/2026_Plasmid_Binning_Bench/hyplas_samples/hyplas_samples.tsv"
declare -r all_samples_outdir="/project/6001426/wg-anoph/benchmarking/2026_Plasmid_Binning_Bench/hyplas_samples/data/assembly/hybrid/unicycler"
...
#
# Usage:
#   > run_sample "$smp_outdir" "$sra_sr_id" "$sra_lr_id"
function run_uni_hybrid {
    local smp_outdir=$1
    local sra_sr_id=$2
    local sra_lr_id=$3
    ...
    # Download long reads
    #
    "${SRA_TOOLS_CMD_PREFIX[@]}" prefetch "$sra_lr_id" --output-directory "$reads_dir"
    "${SRA_TOOLS_CMD_PREFIX[@]}" fastq-dump --split-3 --outdir "$reads_dir" "$reads_dir/$sra_lr_id"

    local fastq_lr="$reads_dir/${sra_lr_id}.fastq"

    gzip -f "$fastq_lr"
    local fastq_lr_gz="${fastq_lr}.gz"

    #
    # Run hybrid assembly
    #
    "${UNICYCLER_CMD_PREFIX[@]}" unicycler -1 "$fastq_1_gz" -2 "$fastq_2_gz" -l "$fastq_lr_gz" -o "$smp_outdir"
...
sra_lr_id=$(get_samples_tsv_cell "$row_line" "$sra_lr_col" "$samples_tsv")
...
echo "SRA SR ID: $sra_sr_id"
echo "SRA LR ID: $sra_lr_id"
echo "----------------------------------------"
echo

run_uni_hybrid "$smp_outdir" "$sra_sr_id" "$sra_lr_id"
# ---------------------------------------------------------------------------- #
