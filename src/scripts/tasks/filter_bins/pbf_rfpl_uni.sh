...
#SBATCH --array=2-1242
#SBATCH --output=logs/pbf_rfpl_uni/%A/%a.out
#SBATCH --error=logs/pbf_rfpl_uni/%A/%a.err
...
bin_dir="$home_dir/DATA/RESULTS/BINNING/PLASBIN_FLOW/RFPLASMID/UNICYCLER/"
...
bins_tsv="$bin_dir/$smp_uid/${smp_uid}_output_file.txt"
filtered_bins_tsv="$bin_dir/$smp_uid/bins_filt.tsv"

echo "$SLURM_JOB_ID $SLURM_ARRAY_TASK_ID $smp_uid filtering PlasBin-Flow+RFPlasmid bins"
